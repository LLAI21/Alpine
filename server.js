...
const checkAuth = (req, res, next) => {
  if (req.session.user) {
    ...
    // L'utilisateur n'est pas connecté, renvoie une réponse non autorisée
...
app.post('/api/articles', async (req, res) => {
  ...
  try {
    const auteur_id = req.session.user ? req.session.user.id : null;

    const nouvelArticle = await knex('articles_blog').insert({
      titre,
      contenu,
